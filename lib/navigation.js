 ...
 * @param {HTMLElement}
 */
function restorePermanentElements(elm) {
  let placeholders = elm.querySelectorAll('[data-reflinks="permanent"]');
  placeholders.forEach(placeholder => {
    ...
    let permanent = permanentElementsById[placeholder.id];
    if(permanent) {
      dom.replaceWith(placeholder, permanent);
    }
  });
};
...
export function visit(path, options = defaultVisitOptions) {
  ...
  return request('get', path).then(res => {
    ...
    dom.emit(document, 'reflinks:before-render', { newRoot });
    ...
    restorePermanentElements(newRoot);
